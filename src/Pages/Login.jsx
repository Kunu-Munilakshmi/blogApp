...
const Login = () => {
    ...
    const handleLogin =async (e) => {
        ...
        if (regex_em.test(email)) {
            setEmailError("");
        }
        else {
            count++;
            setEmailError("email should be Ex: john@gmail.com");
        }
        if (regex_pw.test(password)) {
            ...
            count++;
            setPasswordError("Please Enter Password with atleast 1 uppercase, 1 lowercase, 1 digit and 1 special character");
        }
        if (count == 0) {
    ...
    // Regular Expressions
    const regex_em = /^[\w\.-]+@[\w\.-]+\.\w{2,}$/ // \w->allows [A-Za-z0-9_]
    const regex_pw = /^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[!@#$%^&*]).{8,16}$/
        ...
        <div class="form-container m-4">
            ...
            <form class="form">
                <div class="input-group">
                    ...
                    <input type="email" name="email" id="email" placeholder="" onChange={handleUsername} value={email} />
                </div>
                <p>{emailError}</p>
                <div class="input-group">
            ...
            <p class="signup">Don't have an account?
                <Link rel="noopener noreferrer" to="/signup" class="font-bold">  Sign up</Link>
            </p>
        </div>)
}
